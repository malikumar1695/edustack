...
const operationTabList = [
  { key: "tab1", tab: "Operation Log One" },
  { key: "tab2", tab: "Operation Log Two" },
  { key: "tab3", tab: "Operation Log Three" },
];
const columns = [
  {
    title: "Action Type",
  ...
  if (active) {
    const popoverContent = (
      <div style={{ width: 160 }}>
        ...
        <div style={{ marginTop: 4 }}>Duration: 2 hours 25 minutes</div>
      </div>
    );
    return (
      <Popover
...
export default function Advanced() {
  useEffect(() => {
    document.title = "Advanced Profile - Ilm Admin";
  }, []);
  ...
  const desc1 = (
    <div style={{ position: "relative", paddingTop: 8, fontSize: 14, textAlign: "left" }}>
      Lily Qu
  ...
  const loading = false;
  const contentList: Record<AdvancedState["operationKey"], React.ReactNode> = {
    tab1: (
      <Table
        pagination={false}
        loading={loading}
      ...
      <div>
        <GridContent>
          ...
          <Card
            ...
            variant="borderless"
          >
            <Descriptions style={{ marginBottom: 24 }} items={userInfoItems} />
            <Descriptions
              style={{ marginBottom: 24 }}
              title="Info Group"
